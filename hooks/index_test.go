...
func TestInit(t *testing.T) {
	t.Run("If no error, it will return the hooks", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, Hooks{Usecases: usecs, Resources: reso}, res)
	})
}
...
func (m mock) SetEchoRoute(method string, path string, h echo.HandlerFunc, mid ...echo.MiddlewareFunc) {
	if method == "GET" {
		m.eCtx.Echo().GET(path, h, mid...)
	}

	if method == "POST" {
		m.eCtx.Echo().POST(path, h, mid...)
	}
}
